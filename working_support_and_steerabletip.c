...
int applyTorque(int nodeId, int current, int current_mode_velocity)
{
		canOpenWrite(nodeId, 0x6060, 0, 0x0a);   // Set to current mode (10)	
		canOpenWrite(nodeId, 0x6080, 0, current_mode_velocity); // max speed in current mode
		...
		if(nodeId % 2 == 0)
			canOpenWrite(nodeId, 0x6071, 0, -current);  // apply current
		else
			canOpenWrite(nodeId, 0x6071, 0, current);  // apply current
...
int applyPretension(int pretension_current, long int pretension_velocity)
{
	...
	//{
		...
		//{
			...
			//printf("%ld : Node %d Absulute Velocity: %d, StatusWord: %d\n", clock(), nodeId, abs(nodeData[nodeId].velocity), nodeData[nodeId].statusWord);
			//system_velocity = system_velocity + abs(nodeData[nodeId].velocity);
		//}
...
{ 
	...
	// To disable motors... execute program with a argumanet
	if (argc == 2)
	{
	...
	// turnoff node 11 and 12, not used
	applyTorque(11,0,500);
	applyTorque(12,0,500);
			
	applyTorque (9,300,500);
	applyTorque (10,300,500);
	usleep(200000);
	...
	{	
		...
		printf("press any key to bend tip...\n");
		getchar();
		for (count = 1; count <=10; count++)
		{
			applyTorque(count,150,500);
			usleep(20000);
		}
		//applyTorque(4,300,500);
		//applyTorque(5,20,700);
		
		printf("press any key to reverse...\n");			
		getchar();		
		
		for (count = 1; count <=10; count++)
		{	
			applyTorque(count,0,1000);
			usleep(20000);
		}
		
		//applyTorque(4,20,700);
		//applyTorque(5,300,500);
